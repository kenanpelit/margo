...
| W1.4 | ✅ **Clippy gating shipped.** New `clippy.toml` with `ignore-interior-mutability` for smithay's `Window` / `Output` / `ClientId` / `ObjectId` (matches niri). Cleanup pass took the workspace from **51 → 0 warnings** (auto-fix for ~30 mechanical, manual for filter→find / clamp pattern / collapsible match arms / iter-by-index). Crate-level `#![allow]`s in `margo/src/main.rs` document the cosmetic categories deliberately suppressed (`doc_overindented_list_items`, `too_many_arguments` on 9-arg render helpers, etc.). CI now runs `cargo clippy --workspace --all-targets -- -D warnings` as a gate. |
| W1.5 | ✅ **CONTRIBUTING.md + PR template shipped.** Quick-start build + system deps, code-layout map, lint posture (`cargo clippy ... -D warnings` is a CI gate), test workflow (`cargo insta review`, smoke-winit), commit-message style (conventional commits + why-not-what bodies), tracy-span hints for hot paths, PR review checklist, AI-contribution policy. PR template at `.github/pull_request_template.md` with a checklist that mirrors the local pre-push gate. |
...
| W2.2a | ✅ **Graceful EGL-failure path shipped.** When udev backend bring-up fails, margo no longer prints a one-line anyhow chain — it logs the diagnostic + three actionable fix hints (Mesa install, `/dev/dri/card*` permissions, qemu virgl) and falls back to winit nested mode. Software-render path explicitly noted as W2.2b. ~25 LOC. |
| W2.2b | ⏸ **Full pixman software renderer fallback** *(deferred — bigger than the original estimate).* True pixman fallback needs every custom render element (RoundedBorder, Shadow, ResizeRender, ClippedSurface, OpenClose, LinearComposite — 7 modules) made generic over `R: Renderer + Bind<...>` AND parallel render paths in udev + winit backends. Original ~400 LOC estimate undersells it; realistic scope is ~1500 LOC plus shader rewrites for the SDF border/shadow paths. Re-enter when a user files a "margo doesn't run in my qemu" issue and we have a concrete deployment to test against. |
...
| W2.4 | ✅ **AccessKit a11y tree shipped (first cut).** New `margo/src/a11y.rs` (~230 LOC) ports niri's adapter pattern: AccessKit `Adapter` lives on its own thread because zbus contention on the compositor mainloop deadlocks. `MargoState::a11y: A11yState` field + `publish_a11y_window_list()` helper called from `arrange_all`. Each window becomes an accessible `Node` with `Role::Window` and a `"AppID: Title"` label; the focused window is the tree's `focus`. Build with `--features a11y` (off by default, drops dep tree on lean builds). Orca and AT-SPI consumers can now navigate margo's window list. Future work: per-tag grouping, action handlers (a11y → focus dispatch), live announcements on tag-switch. |
| W2.5 | ✅ **xwayland-satellite mode shipped.** Two new CLI flags: `--xwayland-satellite[=BINARY]` spawns Supreeeme's xwayland-satellite as a separate process (X11 crash can't take margo down); `--no-xwayland` disables X11 entirely (pure-wayland session). Default path stays in-tree (smithay `XWayland::spawn`) so no breaking change for existing users. ~50 LOC. Lifetime coupling is user-side: pair with `systemctl --user link xwayland-satellite.service PartOf=margo-session.target` if you want strict tie-down. |
...
| # | Item | Estimate | Notes |
|---|---|---|---|
| W3.1 | ✅ **HDR Phase 3 — KMS metadata scaffolding shipped.** New `render/hdr_metadata.rs` (~270 LOC). `StaticMetadataDescriptor` ↔ 28-byte kernel blob (HDR_OUTPUT_METADATA), bit-exact spec encoding (CTA-861-G + ST2086) verified by 5 unit tests against published reference values. `EdidHdrBlock` parser decodes panel-advertised peak/avg/min luminance per CTA-861-G §7.5.13. `EotfId` + `InfoFrameType` constants. Activation still upstream-blocked on smithay's `DrmCompositor` exposing `set_hdr_output_metadata`; when that lands the integration is ~30 LOC: queue the blob into the next atomic commit. |
| W3.2 | ✅ **`mctl run <file>` shipped.** New `Run { file }` mctl subcommand + `run_script` dispatch action + `scripting::run_script_file` helper. Script is canonicalised client-side, sent absolute-path through dispatch, eval'd against the same live engine init.rhai uses. Hook registrations inside the script persist after the run (live-edit your hooks). Falls back to standing up a fresh engine if init.rhai never ran. ~140 LOC. |
| W3.3 | ✅ **Rhai plugin packaging shipped.** New `plugin.rs` module (~270 LOC, 4 unit tests). Discovers `~/.config/margo/plugins/<name>/{plugin.toml,init.rhai}` directories at startup; each plugin's script runs against the same engine init.rhai used so plugins can layer hooks on top. Hand-rolled TOML manifest parser (4 fields: name/version/description/enabled — full TOML dep was overkill). `MargoState::plugins` exposes the loaded list for a future `mctl plugin list/enable/disable` workflow. Compile / runtime errors per-plugin don't take down the loader. Niri pattern, but margo's first lift in this area. |
| W3.4 | ✅ **dwl-ipc state-extension fields shipped (state.json side).** Per-output: `scratchpad_visible` / `scratchpad_hidden` counts, plus a `focus_history` array (last 5 focused app_ids, MRU-first). `focus_history` is fed by a new VecDeque on `MargoMonitor` that `focus_surface` pushes to. dwl-ipc-v2 wire protocol stays unchanged (avoids breaking noctalia / waybar-dwl); state.json carries the extension. ~80 LOC. |
| W3.5 | ✅ **Layout switch notification enriched.** `notify_layout` now shows position-in-cycle (`scroller (1/6) → next: tile`) so users navigating the 14-layout catalogue see context. New `notify_layout_state(action, value)` for proportion / gap toggles — same toast theme so the cycle keybind family has consistent feedback. Hooked into `switch_proportion_preset` + `toggle_gaps`. ~30 LOC. |
| W3.6 | ✅ **Per-tag wallpaper hint shipped.** New `tagrule = id:N, wallpaper:path` field on `TagRule` + parser line. `Pertag::wallpapers: Vec<String>` stores per-tag override; `apply_tag_rules_to_monitor` populates from rule. `state.json` exposes both the active-tag wallpaper (`wallpaper`) and the per-tag map (`wallpapers_by_tag`) so wallpaper daemons (swaybg-wrapper, noctalia, custom) can swap on tag-switch. niri can't host this — no tags. |
...
| W4.4 | ✅ **`mctl migrate` shipped (Hyprland + Sway).** New `margo-ipc/src/migrate.rs` (~430 LOC, 9 unit tests) parses hyprland.conf or sway/config and emits an equivalent margo `config.conf` to stdout (or `-o PATH`). Auto-detects format from path heuristics + content sniff (`bind = ...` → hyprland, `bindsym ...` → sway); override with `--from`. Translates the high-value subset: keybinds + spawn lines + workspace → tag bitmask conversions + modifier names (`Mod4`/`SUPER` → `super`) + key aliases (`RETURN`/`return` → `Return`). Window rules / animations / monitor topology stay manual — auto-translating them would invent semantics the source compositor doesn't actually mean. **Niri's KDL is intentionally out-of-scope**: niri's workspaces+scrolling design doesn't map onto margo's tag-based one without inventing wrong semantics. Unconvertible source lines emit a warning to stderr with the line number, while still writing every translatable line. Offline (no compositor IPC). |
