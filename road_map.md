...
- **W4.2 — Phase 1 + 2 shipped, more queued.** First two passes extract 8 handlers into `margo/src/state/handlers/` (738 LOC moved): `xdg_decoration` (62), `session_lock` (96), `xdg_activation` (113), `layer_shell` (262), `color_management` (21), `idle` (50), `pointer_constraints` (77), `input_method` (57). Pattern: each submodule reaches into `MargoState` via `crate::state::MargoState`; the `delegate_*!` macros stay co-located with their impls. state.rs went 7651 → 6992 LOC (first time under 7000). Remaining big extractions queued: `xdg_shell` (~417 LOC), `x11/xwm` (~150 LOC), `output_management` (~170 LOC), `selection` (~110 LOC), `screencopy` (~190 LOC), `compositor` (~150 LOC), `dmabuf+drm_syncobj` (~80 LOC). Behaviour-preserving — all 40 layout tests + clippy gate stay green at every step.
