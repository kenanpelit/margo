...
| **P15** 🚧 | `wp_fifo_v1` + `wp_commit_timing_v1` | Bindings landed in `74a0edb`, but the globals are intentionally not advertised: Smithay's managed implementation installs commit barriers and Margo does not yet release them from a per-output FIFO/deadline scheduler. Advertising the incomplete path can stall Chromium after a hidden-tag round trip. | needs presentation-driven barrier release + commit-deadline scheduling before re-advertisement |
