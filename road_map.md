...
> **Last updated:** 2026-09-04 — now at **v3.1.0**. Phase 1 closed at v0.1.6; Phase 2 (Quality & growth) engineering bars met; Phase 3 (polish + ecosystem, §15.11) is the active phase. The legacy **dwl-ipc-v2 + `state.json` IPC** was removed (2026-06-01) for a single Unix control socket (`MARGO_SOCKET`, `get`/`watch state`/`dispatch`). The first-party binary family is now **mshell, mshellctl, mctl, mlock, mlogind, mgreet, mpower, mlayout, mscreenshot, mkeys, mvpn, mcal, mtune, mpicker, mdots, mvisual, mwizard, margo-portal, start-margo** — `mplay` folded into native `mshellctl play`/`media` in **v3.0.0** (with automatic `mctl config` history), and **v3.1.0** added `mtune` (a folder-first music player: MPRIS + tray + its own bar pill/menu + `mshellctl mtune` CLI) and a **floating** (stacking) layout. The shell carries a control center, native AI assistant, alarm clock, privacy indicator, MRU window switcher, dock, keybinds editor, GNOME-parity Settings, the WASM plugin tier (`mplugin-sdk`), and first-login config bootstrap. Wayland protocol parity (§15.10): **15/17 Tier items + 5 bonus** — the last 2 (P11 drm-lease, P13 tearing) are blocked by margo architecture / smithay capability, not effort. Packaged on the **AUR** (`margo-git`). Side-by-side audit in `docs/protocol-comparison.md`.
> **Branch:** `main` (single-branch — Rust port complete; no C code remains in the repo)
> **Status:** Daily-driver Wayland compositor + first-party desktop (shell, login, lock, portals, VPN, OSK, calendar, music) at niri-class parity and beyond, released as **3.x**. What's open is upstream-blocked, hardware-gated, on-demand features, or community growth (§15).
>
...
| Wayland protocol parity (P1–P17) | Tier 1: 5/5 (P1–P5). Tier 2: 5/6 (P6, P7, P8, P9, P10). Tier 3: 5/6 (P12, P14, P15, P16, P17 — **P12 `output_power` shipped in 1.0.2**). Bonus: 5 smithay-natives (xwayland-keyboard-grab, toplevel-icon, system-bell, pointer-warp, toplevel-tag) + socket-era additions (foreign-toplevel write-side, `ext_workspace_v1`, `virtual_pointer`). Remaining 2: P11 drm-lease (architecture-blocked) / P13 tearing (upstream-blocked). Side-by-side audit in `docs/protocol-comparison.md`. | ✅ 15/17 + 5 bonus |
...
> **⚠️ Superseded (2026-06-01):** dwl-ipc-v2 and `state.json` were **removed**.
> The single IPC is now a Unix control socket (path in `MARGO_SOCKET`) with
> `get` / `watch state` / `dispatch` verbs; third-party bars use
> `ext_workspace_v1` + foreign-toplevel. The text below is archaeology.
...
| **W1.7** Design-language lint gate | ✅ shipped (1.0.4 era) | `scripts/design-lint.sh` enforces `DESIGN.md §15` L1–L7 as **hard gates** in `ci.yml` (hardcoded hex, raw ≥4px spacing/radius/font, `--radius-widget` outside bar/frame, literal motion durations, primary-surface `Popover` in a bar widget, `add_css_class("")`, `DragSource`/`DropTarget` row reorder), with the §15-carved exceptions (0–3px micro-insets incl. shorthand, calc()/var() token-anchored values, `PopoverMenu` context menus, comment stripping). All seven gated hard at once — the pre-flip audit found only 3 real hits (privacy menu rows on `--radius-widget` → `--radius-sm`; two deliberate 5px launcher paddings → `calc(var(--space-1) + 1px)`), fixed in the same commit. L8 stays advisory (grep can't prove the thread); L9 is the existing fmt/clippy steps. |
...
| **B10** | `isfloating:1` rule with no size hint → 0×0 invisible window | ✅ shipped | `apply_window_rules` only synthesised `float_geom` when the rule carried `width`/`height`/`width_fraction`/`height_fraction`/`offset`. A rule with only `isfloating:1` set `is_floating=true` but left `float_geom = (0,0,0,0)`, and arrange's `if float_geom.width > 0` skipped the apply — toplevel configured at 0×0, invisible. Fix in `ff4c30f` adds a post-loop fallback: if `is_floating` ended true and `float_geom` is still empty, synthesise from a default WindowRule (60 % of work_area centred). Repro user-reported during screenshot editor sessioning. |
| **B11** | Margo Tags pill row empty at session start until first window opens | mid | **Open after 5 attempts.** Watcher fixes (7277a95) extended cold-start poll + immediate-fire-on-subscribe; active-pill fallback fix (142317e) made get_active_workspaces survive empty state; brute-force glib timer (3df970e) re-issues SetItems every 500 ms; state.json bootstrap synthesis (7b90074) builds 9 placeholder Workspace Arcs directly, bypassing the margo-client reactive cache. **None of these surfaced pills.** First window opening still kicks them in. Next-time: need live runtime tracing (`RUST_LOG=mshell_margo_client=debug,mshell_frame=debug mshell 2>&1 \| grep -i workspace`) to see what's actually firing. Suspect: DynamicBox launch / mounting issue (controller built but widget never attached), OR factory.create silently dropping items (returns Box<dyn> but the actual controller spawn never happens). Likely needs a smaller test reproducing the DynamicBox empty→9 transition in isolation. |
...
| **P15** ✅ | `wp_fifo_v1` + `wp_commit_timing_v1` | Presentation-pacing protocols, advertised again in 1.1.9 **with** their scheduler: `margo/src/state/pacing.rs` signals FIFO barriers at every per-output present (real vblank, empty present, estimated vblank), releases hidden-tag/locked/disabled surfaces from the frame-callback fallback tick, and honours commit-timing deadlines up to the next refresh plus an exact one-shot deadline wake armed from the surface pre-commit hook. Gives Mesa Vulkan WSI real `VK_PRESENT_MODE_FIFO_KHR` and video players compositor-driven pacing. Regression-tested in `margo/src/tests/pacing.rs`, incl. the hidden-tag Chromium stall that forced the 2026-07-13 withdrawal. | managed smithay states + `state/pacing.rs` barrier-release scheduler |
...
| **Automatic login (mlogind + Settings)** | GNOME-style passwordless auto-login. **Design (worked out 2026-05-26):** (1) add `autologin_user` (empty = off) to the `mlogind` `Config` macro in `config.rs` + an empty default in `extra/config.toml`; (2) branch in `main.rs` *before* the TUI — when set, take the passwordless path (skip PAM `authenticate()`, keep `acct_mgmt` + `setcred` + `open_session` — the standard DM autologin route) and launch via `PostLoginEnvironment::spawn`, falling back to the normal greeter on session exit so logout still prompts; (3) a `mlogind set-autologin [user]` CLI subcommand (mirrors the existing `sync-theme`) that edits `/etc/mlogind/config.toml`; (4) a new Settings → Login page (switch + user dropdown from `read_users`) that runs `pkexec mlogind set-autologin <user>`. **Security/test caveat:** passwordless = physical access ⇒ that user's session; the PAM session-open path can't be CI-tested (lockout risk) so it needs live login verification with the TTY/`--preview` fallback kept. | mid–high (login-manager auth change + Settings page; security-sensitive, manual test) |
| **Settings → Users: fingerprint enrolment** | GNOME's "Fingerprint Login" enrol flow inside the Users page — guided multi-touch enrol via fprintd (the `mshell-auth` client already does *verify*; enrol is the new bit). Gated on `fprintd` being installed. Pairs with the **mlogind async fingerprint UI** row above (greeter-side verify). | mid (~150 LOC + fprintd enrol API) |
