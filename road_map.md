...
| **W1.7** Design-language lint gate | 🔜 planned | Make `DESIGN.md §15` mechanically enforced: a `scripts/design-lint.sh` grep gate wired into `ci.yml` (alongside fmt/clippy) that fails the build on the "must-not-appear" patterns L1–L7/L9 (hardcoded hex, raw spacing/radius/font px, `--radius-widget` outside bar/frame, literal motion durations, primary-surface `Popover` in a bar widget, `add_css_class("")`, `DragSource`/`DropTarget` row reorder). L8 (main-thread blocking) stays advisory (grep can't prove the thread). Ship conservatively: the high-certainty rules (L5/L6/L7) hard-gate first; the SCSS rules (L1/L2/L4) run in report-only mode until the tree is confirmed clean (current audit: clean bar the one `--error` hex fallback, already removed), then flip to hard. The `§15` table already encodes each grep + its false-positive exceptions. |
