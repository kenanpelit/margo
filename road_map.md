...
| **W1.6** Integration test fixture | ✅ shipped (first cut) | Port of niri's `src/tests/{fixture,server,client}.rs` pattern to margo. New `margo/src/tests/` module gated on `#[cfg(test)]`: `server.rs` (~70 LOC) wraps a real `MargoState` driven by calloop; `client.rs` (~180 LOC) is a `wayland-client::Connection` with `prepare_read → read → dispatch_pending` loop and per-`ClientState` `Dispatch<WlRegistry/WlCallback/WlDisplay>` impls; `fixture.rs` (~120 LOC) interleaves server + client dispatch via `roundtrip(id)` with a 200-turn upper bound to surface deadlocks as panics instead of hangs. First three smoke tests in `globals.rs`: `fresh_client_sees_all_required_globals` (30 globals, deliberately excludes `xwayland_shell_v1` / `wp_color_manager_v1` / dmabuf / drm_syncobj which are gated on a real backend), `second_client_sees_the_same_globals_as_the_first` (registration-order regression), `fixture_dispatches_without_clients` (pure-server smoke). Test count: 85 → 88 (+3); future protocol-handler tests pile onto the same harness without rebuilding the server skeleton each time. |
