...
> **Last updated:** 2026-06-12 — **1.0 shipped**: v1.0.0 tagged 2026-06-09, now at **v1.0.6**. Phase 1 closed at v0.1.6; Phase 2 (Quality & growth) engineering bars met; Phase 3 (polish + ecosystem, §15.11) is the active phase. Since the 0.8 snapshot: the legacy **dwl-ipc-v2 + `state.json` IPC was removed** (2026-06-01) in favour of a single Unix control socket (`MARGO_SOCKET`, `get`/`watch state`/`dispatch`); the first-party binary family grew to **mvpn, mkeys, mplay, mpower, mlogind, margo-portal, mwizard**; the shell gained a control center, native AI assistant, alarm clock, privacy indicator, MRU window switcher, dock, keybinds editor, GNOME-parity Settings pages, the WASM plugin tier (`mplugin-sdk`), and first-login config bootstrap. Wayland protocol parity (§15.10): **15/17 Tier items + 5 bonus** — `output_power` shipped in 1.0.2; the last 2 (P11 drm-lease, P13 tearing) are blocked by margo architecture / smithay capability, not effort. Packaged on the **AUR** (`margo-git`). Side-by-side audit in `docs/protocol-comparison.md`.
> **Branch:** `main` (single-branch — Rust port complete; no C code remains in the repo)
> **Status:** Daily-driver Wayland compositor + first-party desktop (shell, login, lock, portals, VPN, OSK) at niri-class parity and beyond, released as **1.0.x**. What's open is upstream-blocked, hardware-gated, on-demand features, or community growth (§15).
>
