...
| **Settings → Users: automatic login** | GNOME's auto-login toggle. Needs an `mlogind` autologin setting first (config field + the greeter honouring it), then a Switch in Settings → Users writes it. The Users page (e97e53b) already covers avatar / full name / account type / password / add-remove via pkexec — this is the missing GNOME-parity row. | mid (mlogind config + greeter + Settings wire) |
