//! Pure window-placement helpers (no I/O) for `mshellctl play snap`.
//! Ported from `mplay`'s `geometry.rs` (itself a port of `margo-mpv.sh`'s
//! `cmd_move` corner-cycle math), so it stays unit-testable in isolation.
...
/// The four floating-window resting corners, cycled by `mshellctl play snap`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Corner {
    TopLeft,
    TopRight,
