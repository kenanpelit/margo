...
//! script; also ported into `mplay`'s `ytdl_shim.rs`, this is the same
//! logic living under `mshellctl` instead). mpv's `ytdl_hook` invokes
//! this as its `ytdl_path`; we forward to `yt-dlp` with hardened YouTube
//! options, a browser user-agent, a cookie-file path, and an anti-bot
//! client fallback. yt-dlp's **stdout** (the JSON mpv consumes) is passed
//! through untouched; only **stderr** is inspected to decide whether to
//! retry with the anti-bot client.
...
fn try_antibot(tail: &[String]) -> bool {
    eprintln!("[mshellctl ytdlp] anti-bot fallback: youtube client tv_simply,web");
...
/// Entry point for the hidden `mshellctl play __ytdlp …` subcommand.
...
pub fn run(tail: &[String]) -> i32 {
    ...
    if std::fs::metadata(&cookies).is_ok() {
        eprintln!("[mshellctl ytdlp] using cookies file: {cookies}");
        ...
        if is_antibot_error(&err) {
            eprintln!("[mshellctl ytdlp] bot-check: trying anti-bot client…");
    ...
    eprintln!("[mshellctl ytdlp] no cookies available; proceeding without auth");
