...
//! Ported from `mplay`'s `paper` module.
//!
//! `run()` forks into its own background process when `daemon: true` (the
//! caller — `mshellctl` — exits immediately in that case), exactly like
//! `mplay`'s own wallpaper engine does. So this never shares a process,
//! GL context, or Wayland connection with anything else: `mshellctl`
//! itself is a short-lived CLI, not the long-running `mshell` GTK4 shell
//! daemon.
//!
//! The pidfile directory (`$XDG_RUNTIME_DIR/mplay/`) is intentionally the
//! *same* one `mplay`'s own wallpaper engine uses — `wallpaper stop` from
//! either tool can stop a daemon the other one started, which is the
//! right behaviour while both are installed side by side.
