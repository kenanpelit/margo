...
pub fn dispatch_action(state: &mut MargoState, action: &str, arg: &Arg) -> DispatchOutcome {
    ...
    match action {
        ...
        "mru_next" | "mruwindow" => {
            state.mru_advance_args(arg.v.as_deref(), arg.v2.as_deref(), MruDirection::Forward)
        }
        "mru_prev" => {
        ...
        "open_grid_overview" => state.open_overview(),
        "close_grid_overview" => state.close_overview(None),
