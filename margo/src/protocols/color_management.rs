...
impl ColorManagementState {
    pub fn new<D, F>(display: &DisplayHandle, enabled: bool, filter: F) -> Self
    where
    ...
    {
        // Phase 1.5 history: the global was fully withheld (niri-style)
        // after real client probes crashed against the early Phase 1
        // event flow — mpv `--vo=gpu` walks `get_surface_feedback →
        // get_preferred → get_information` and kicks the connection
        // when the response chain isn't 100% spec-correct; Chromium
        // ozone follows the same path. The full info-event sequence
        // (Hyprland-pattern, all 8 events + done) has since landed
        // below, so the global is now **opt-in via the
