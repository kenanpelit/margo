...
//!
//! Companion to margo's already-shipped `zwp_virtual_keyboard_v1`:
//! remote-desktop / accessibility / `wtype --click`-style tools inject
//! pointer motion, buttons and axis. Smithay has no impl; ported nearly
//! verbatim from niri's `protocols/virtual_pointer.rs` (the synthetic
//! `InputBackend` is generic and carries no compositor-specific types).
//!
//! Events are fed into margo's normal input path via
//! `crate::input_handler::handle_input` (handler in
//! `state/handlers/virtual_pointer.rs`). Closes road_map §15.10 P7.
...
impl VirtualPointer {
    ...
    fn mutate_axis_frame(&self, time: Option<u32>, f: impl FnOnce(AxisFrame) -> AxisFrame) {
        ...
        *frame = frame
            .or(time.map(|t| AxisFrame::new(InputTime::from_millis(t))))
            .map(f);
    }
}
    ...
    fn request(
        ...
        match request {
            ...
            zwlr_virtual_pointer_v1::Request::Button {
                ...
                // wlroots treats `state` as a C boolean: Released stays
                // released, anything else (including invalid) is pressed.
