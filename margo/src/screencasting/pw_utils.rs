...
/// `tracy_client` shim — niri uses Tracy profiler spans via the
/// `tracy_client::span!` macro. Margo doesn't link tracy; we
/// expose a no-op `span!` so the ported code keeps the call sites
