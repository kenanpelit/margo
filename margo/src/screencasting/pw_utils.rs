...
use crate::dbus::cast_ids::{CastSessionId, CastStreamId};
use crate::dbus::mutter_screen_cast::{self, CursorMode};
use crate::screencasting::render_helpers::{
    clear_dmabuf, encompassing_geo, render_and_download, render_to_dmabuf,
};
use crate::screencasting::{CastRenderElement, CastTarget};
use crate::state::MargoState as State;
use crate::utils::get_monotonic_time;

/// `tracy_client` shim — niri uses Tracy profiler spans
/// throughout pw_utils via the `tracy_client::span!` macro.
/// margo doesn't link tracy; this shim makes the macro a no-op
/// so the ported code keeps the call sites without instrumentation
/// overhead. Drop this if/when margo gets a real profiler hookup.
mod tracy_client {
    macro_rules! span {
        ($($t:tt)*) => {
            ()
