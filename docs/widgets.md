...
| `AudioRoute` | Audio Route | One-click switch of the whole default audio path between your built-in output and a headset. Detects the headset from PipeWire's device metadata (Bluetooth / USB, or a combo-jack port), so it works across machines without hardcoded names; shows the live route (Built-in / Headset) and flips it on click. Hidden when there's no headset to switch to. The mic follows the speaker by default — toggle that off in Settings → Widgets → Audio Route. |
