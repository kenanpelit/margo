...
//!
//! The daemon listens on `$XDG_RUNTIME_DIR/mshell.sock`.
//! The same binary acts as a client via `mshell msg <command>`.
...
pub fn socket_path() -> Result<PathBuf> {
    if let Some(dir) = std::env::var_os("XDG_RUNTIME_DIR") {
        return Ok(PathBuf::from(dir).join("mshell.sock"));
    }
    let uid = unsafe { libc::getuid() };
    Ok(PathBuf::from(format!("/tmp/mshell-{uid}.sock")))
}
...
pub fn run_client(cmd: &IpcCommand) -> Result<()> {
    ...
    let mut stream = UnixStream::connect(&path)
        .with_context(|| format!("connect to {} — is mshell running?", path.display()))?;
...
enum ListenerError {
    /// Another mshell instance is already listening on the socket.
    AlreadyRunning,
    Other(anyhow::Error),
}
...
/// primary's fd and break `mshell msg` until it's restarted.
...
fn init_listener() -> Option<tokio::net::UnixListener> {
    let std_listener = match create_listener() {
        ...
        Err(ListenerError::AlreadyRunning) => {
            log::warn!(
                "another mshell instance owns the IPC socket; this instance will run without IPC"
            );
            return None;
        }
