...
//! the config file).
//!
//! This is *the* single source: `mctl actions` and the completion
//! generators render it, the config `validator` checks every bind's
//! action against it (W005), and a test in `margo` asserts
//! `dispatch_action`'s match arms cover every entry here (and vice
//! versa) so the list can't silently drift from the dispatcher.
//!
//! It lives in `margo-config` — the leaf crate `margo`, `mctl` and the
//! shell all already depend on — because config validation needs it
//! and `margo-config` can't depend on `margo` (the compositor binary)
//! or `mctl` without a workspace cycle.
//!
...
/// Whether `name` is a canonical action name or one of its aliases —
/// i.e. something `dispatch_action` will actually handle. Used by the
/// config validator (W005). Case-sensitive, matching the dispatcher's
/// own `match action`.
pub fn is_known(name: &str) -> bool {
    ACTIONS
        .iter()
        .any(|a| a.name == name || a.aliases.contains(&name))
}
