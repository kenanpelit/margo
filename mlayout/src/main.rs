...
fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Cmd::Init { .. } | Cmd::New { .. } | Cmd::Suggest { .. } => {
            unreachable!("Init/New/Suggest are short-circuited above before this match")
        }
    }
}
...
fn cmd_init(config_dir: &Path, name: &str, yes: bool, force: bool, no_reload: bool) -> Result<()> {
...
fn cmd_suggest(
    ...
    //    pick prompt below.
    let preview_layouts: Vec<parser::Layout> = presets.iter().map(preset_to_layout).collect();
    println!("\nAvailable arrangements:");
    for (i, (preset, layout)) in presets.iter().zip(preview_layouts.iter()).enumerate() {
        println!("\n  {}. {} ({})", i + 1, preset.name, preset.slug);
...
fn cleanup_auto_generated_presets(config_dir: &Path) -> Result<Vec<String>> {
    ...
    for entry in entries {
        ...
        let is_autogen = body.lines().take(3).any(|l| {
            let t = l.trim();
            t == presets::AUTOGEN_MARKER.trim() || t.starts_with("# Generated by `mlayout suggest`")
        });
        if is_autogen && std::fs::remove_file(&path).is_ok() {
            let slug = name_str
...
fn cmd_new(
    ...
    if activate_now {
        ...
        println!("\nRun `mlayout set {}` to activate this layout.", name);
    }
    Ok(())
}
...
fn config_already_sourced(path: &Path) -> Result<bool> {
    ...
    Ok(body.lines().any(|l| is_layout_source_line(l.trim_start())))
}
...
fn cmd_list(
    ...
    if layouts.is_empty() {
        ...
        println!("Create one or more layout_<name>.conf files there to use mlayout.");
        return Ok(());
    }
...
fn cmd_set(config_dir: &Path, layouts: &[Layout], needle: &str, no_reload: bool) -> Result<()> {
...
fn cmd_cycle(config_dir: &Path, layouts: &[Layout], step: i32, no_reload: bool) -> Result<()> {
    if layouts.is_empty() {
...
fn cmd_preview(config_dir: &Path, layouts: &[Layout], needle: Option<&str>) -> Result<()> {
...
fn cmd_pick(config_dir: &Path, layouts: &[Layout], no_gui: bool, no_reload: bool) -> Result<()> {
    if layouts.is_empty() {
        bail!("no layouts found in {}", config_dir.display());
    }

    if !no_gui && let Some(picker) = detect_graphical_picker() {
        return run_graphical_pick(&picker, config_dir, layouts, no_reload);
    }
    run_inline_pick(config_dir, layouts, no_reload)
}

fn run_inline_pick(config_dir: &Path, layouts: &[Layout], no_reload: bool) -> Result<()> {
    println!();
    let active = current_slug(config_dir);
    for (i, layout) in layouts.iter().enumerate() {
        let active_marker = if Some(&layout.slug) == active.as_ref() {
...
fn detect_graphical_picker() -> Option<PickerCmd> {
    if which("fuzzel") {
        return Some(PickerCmd {
            binary: "fuzzel",
            args: vec!["--dmenu".into(), "--prompt".into(), "layout: ".into()],
        });
    }
    if which("wofi") {
        return Some(PickerCmd {
    ...
    if which("rofi") {
        return Some(PickerCmd {
            binary: "rofi",
            args: vec!["-dmenu".into(), "-p".into(), "layout".into(), "-i".into()],
        });
    }
    None
}
...
fn activate(config_dir: &Path, layout: &Layout) -> Result<()> {
    ...
    let temp = config_dir.join(format!("{}.{}.tmp", ACTIVE_LINK, std::process::id()));
