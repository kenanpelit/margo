...
#[derive(Debug)]
pub(crate) enum PowerMenuWidgetOutput {}

pub(crate) struct PowerMenuWidgetInit {}
...
impl Component for PowerMenuWidgetModel {
    type CommandOutput = PowerMenuWidgetCommandOutput;
    type Input = PowerMenuWidgetInput;
    type Output = PowerMenuWidgetOutput;
    type Init = PowerMenuWidgetInit;

    view! {
        #[root]
        gtk::Box {
            add_css_class: "power-menu-widget",
            ...
            gtk::Box {
                ...
                gtk::Image {
                    ...
                    set_icon_name: Some("power-profile-balanced-symbolic"),
    ...
    fn init(
        ...
        {
            let s = sender.clone();
            idle_button.connect_clicked(move |_| s.input(PowerMenuWidgetInput::ToggleIdleInhibit));
        }
        ...
        spawn_idle_inhibitor_watcher(&sender, || PowerMenuWidgetCommandOutput::IdleStateChanged);
    ...
    fn update(
        ...
        match message {
            ...
            PowerMenuWidgetInput::CycleProfile => {
            ...
            PowerMenuWidgetInput::ToggleIdleInhibit => {
    ...
    fn update_cmd_with_view(
        ...
        match message {
            ...
            PowerMenuWidgetCommandOutput::IdleStateChanged => {
...
fn make_profile_button(profile: Profile) -> gtk::Button {
    ...
    gtk::Button::builder()
        .child(&inner)
        .css_classes(vec!["ok-button-surface", "power-profile-button"])
        .hexpand(true)
        .build()
}
...
fn make_control_button(icon: &str, text: &str) -> (gtk::Button, gtk::Image, gtk::Label) {
    ...
    let button = gtk::Button::builder()
        .child(&inner)
        .css_classes(vec!["ok-button-surface", "power-control-button"])
        .hexpand(true)
        .build();
    (button, img, label)
}

fn sync_view(model: &PowerMenuWidgetModel) {
    let s = &model.state;
    ...
    for (p, btn) in &model.profile_buttons {
        let mut classes = vec!["ok-button-surface", "power-profile-button"];
        if Some(*p) == s.profile {
