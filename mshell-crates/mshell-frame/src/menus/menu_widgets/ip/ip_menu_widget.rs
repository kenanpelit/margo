//! Public IP menu widget — content surface for `MenuType::Ip`.
//!
//! Mirrors the noctalia `ip/Panel.qml` layout: a hero card with
...
impl std::fmt::Debug for IpMenuWidgetModel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("IpMenuWidgetModel")
...
#[derive(Debug)]
pub(crate) enum IpMenuWidgetInput {
    RefreshNow,
    CopyIp,
    OpenInBrowser,
}
...
#[derive(Debug)]
pub(crate) enum IpMenuWidgetOutput {}

pub(crate) struct IpMenuWidgetInit {}

#[derive(Debug)]
pub(crate) enum IpMenuWidgetCommandOutput {
    Refreshed(IpSnapshot),
}

#[relm4::component(pub(crate))]
impl Component for IpMenuWidgetModel {
    type CommandOutput = IpMenuWidgetCommandOutput;
    type Input = IpMenuWidgetInput;
    type Output = IpMenuWidgetOutput;
    type Init = IpMenuWidgetInit;

    view! {
        #[root]
        gtk::Box {
            add_css_class: "ip-menu-widget",
            ...
            gtk::Box {
                ...
                open_button_widget -> gtk::Button {
                    ...
                    connect_clicked[sender] => move |_| {
                        sender.input(IpMenuWidgetInput::OpenInBrowser);
                    },
                },
            },
        }
    ...
    fn update_cmd_with_view(
        ...
        match message {
            IpMenuWidgetCommandOutput::Refreshed(snap) => {
...
fn sync_view(model: &IpMenuWidgetModel) {
