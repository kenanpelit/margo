...
// ── Service-state probes ──────────────────────────────────────
//
// The live system-state readers (battery, disk, twilight, UFW, podman,
// Wi-Fi, Bluetooth, audio, VPN, Valent, airplane) live in `tile_probes.rs`.
mod tile_probes;
use tile_probes::*;

#[cfg(test)]
mod tests {
    use super::tile_probes::{twilight_mode_label, twilight_subtitle, valent_state_from_report};
    use super::{TileId, ordered_tile_ids};
