//! CPU Dashboard menu widget — rich CPU + memory + load panel
//! that lives in the layer-shell menu stack (opens contiguous
//! with the bar like every other widget menu, not as a free-
//! floating popover).
//!
//! Content: a hero card with the aggregate CPU % + package
//! temp (severity-tinted), a per-core load bar grid, a RAM
//! usage row, and the 1/5/15 m load averages line.
...
fn severity_class(cpu: u32, temp: i32) -> &'static str {
    let cpu_state = if cpu >= CPU_DANGER_PERCENT { 2 }
        else if cpu >= CPU_WARN_PERCENT { 1 } else { 0 };
    let temp_state = if temp >= TEMP_DANGER_CELSIUS { 2 }
        else if temp >= TEMP_WARN_CELSIUS { 1 } else { 0 };
...
fn cpu_only_severity(cpu: u32) -> &'static str {
    if cpu >= CPU_DANGER_PERCENT { "danger" }
    else if cpu >= CPU_WARN_PERCENT { "warn" }
    else { "calm" }
}
...
impl Component for CpuDashboardMenuWidgetModel {
    ...
    fn init(
        ...
        // 2 s self-cancelling poll (same pattern as the bar pill).
...
// ── Local /proc readers (no module dep churn) ───────────────────
