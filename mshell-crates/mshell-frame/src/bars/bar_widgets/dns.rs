//! DNS / VPN bar pill — port of the noctalia `dns` plugin's bar
//! half.
//!
//! Render-only widget. Polls DNS / VPN / Blocky state every 30 s
//! and draws an icon + tooltip from the result. Click emits
//! `DnsOutput::Clicked`; `frame.rs` toggles the layer-shell
//! `MenuType::Dns` menu in response (the menu UI lives in
//! `menu_widgets/dns/dns_menu_widget.rs`).
//!
