//! Public-IP bar pill — port of the noctalia `ip` plugin's bar
//! half.
//!
//! Render-only widget. Polls ipinfo.io every 300 s, draws a globe
//! icon + tooltip. Click emits `IpOutput::Clicked`; frame toggles
//! the layer-shell `MenuType::Ip` (the detail panel lives in
//! `menu_widgets/ip/ip_menu_widget.rs`).
//!
...
#[derive(Debug)]
pub(crate) enum IpInput {
    Clicked,
}

#[derive(Debug)]
pub(crate) enum IpOutput {
    Clicked,
}

pub(crate) struct IpInit {}

#[derive(Debug)]
pub(crate) enum IpCommandOutput {
    Refreshed(IpSnapshot),
}

#[relm4::component(pub)]
impl Component for IpModel {
    type CommandOutput = IpCommandOutput;
    type Input = IpInput;
    type Output = IpOutput;
    type Init = IpInit;

    view! {
        #[root]
        gtk::Box {
            set_css_classes: &["ok-button-surface", "ok-bar-widget", "ip-bar-widget"],
            set_hexpand: false,
            set_vexpand: false,
    ...
    fn update_cmd_with_view(
        ...
        match message {
            IpCommandOutput::Refreshed(snap) => {
...
fn apply_visual(image: &gtk::Image, root: &gtk::Box, snap: &IpSnapshot) {
...
/// One ipinfo.io fetch wrapped in a `IpSnapshot`. Exposed
/// pub(crate) so the menu widget can re-run it on demand.
pub(crate) async fn fetch_snapshot() -> IpSnapshot {
