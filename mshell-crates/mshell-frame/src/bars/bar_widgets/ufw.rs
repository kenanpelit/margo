...
//!
//! Click emits `UfwOutput::Clicked`; `frame.rs` catches that and
//! toggles the proper layer-shell `MenuType::Ufw` menu (built by
//! `menus::menu`, positioned per `config.menus.ufw_menu.position`).
...
#[derive(Debug)]
pub(crate) struct UfwModel {
    summary: UfwSummary,
}

#[derive(Debug)]
pub(crate) enum UfwInput {
    Clicked,
}

#[derive(Debug)]
pub(crate) enum UfwOutput {
    Clicked,
}

pub(crate) struct UfwInit {}

#[derive(Debug)]
pub(crate) enum UfwCommandOutput {
    Refreshed(UfwSummary),
}

#[relm4::component(pub)]
impl Component for UfwModel {
    type CommandOutput = UfwCommandOutput;
    type Input = UfwInput;
    type Output = UfwOutput;
    type Init = UfwInit;

    view! {
        #[root]
        gtk::Box {
            set_css_classes: &["ok-button-surface", "ok-bar-widget", "ufw-bar-widget"],
            set_hexpand: false,
            set_vexpand: false,
    ...
    fn update_cmd_with_view(
        ...
        match message {
            UfwCommandOutput::Refreshed(summary) => {
...
fn apply_visual(image: &gtk::Image, root: &gtk::Box, s: &UfwSummary) {
    ...
    // `_ufw.scss` (.ufw-bar-widget.active / .inactive / .unknown)
