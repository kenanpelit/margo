...
/// mlogind's six theming slots so the login screen matches the wallpaper.
pub fn to_mlogind_variables(theme: &MatugenTheme) -> String {
    ...
    format!(
        ...
        background = col!(c.surface),
        foreground = col!(c.on_surface),
        neutral = col!(c.surface_variant),
        dim = col!(c.on_surface_variant),
        error = col!(c.error),
    )
}
