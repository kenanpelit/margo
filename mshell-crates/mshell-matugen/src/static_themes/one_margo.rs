...
/// One Margo — [`super::margo::margo`] with One Dark's foreground instead
/// of Margo's own. Every other slot (background, surface, accents, ANSI)
/// is identical to Margo; only the text/icon colour changes.
///
/// Slot summary:
///
/// | Slot                | Hex      |
...
/// | foreground (text)   | `#ABB2BF` (One Dark's `on_surface`) |
/// | primary             | `#BD93F9` |
/// | secondary           | `#FF79C6` |
/// | tertiary            | `#8BE9FD` |
/// | error               | `#FF5555` |
pub fn one_margo(mshell: MShell) -> MatugenTheme {
    // Foreground (text + icon) — One Dark's `on_surface`/`on_background`.
    // Shared across all on-surface / on-background / inverse-surface /
    // base05-07 slots so a single tweak rolls through every text-bearing
    // widget, same as Margo's own FG constant.
    const FG: &str = "#ABB2BF";
    ...
    MatugenTheme {
        ...
        colors: Colors {
            ...
            surface_container_high: color("#3e4257"),
            surface_container: color("#34374a"),
            surface_container_low: color("#2b2d39"),
            surface_container_lowest: color("#232530"),
